...
public static class Ascon128v12
{
	...
	/// </summary>
	public const ulong ASCON_128_IV = 0x80400c0600000000ul;

	/// <summary>
	/// How many bytes are processed at one time (if available)
	/// </summary>
	public const int ASCON_AEAD_RATE = 8;
	...
	private static void ascon_encrypt(ref ascon_state_t s, byte[] c, byte[] m, int mlen) 
	{
		...
		if (mlen > 0) 
		{
			s.x[0] ^= LOAD(m, mOffset, mlen);
			STORE(c, cOffset, s.x[0], mlen);
		}
	...
	private static void ascon_decrypt(ref ascon_state_t s, byte[] m, byte[] c, int clen) 
	{
		...
		if (clen > 0) 
		{
			...
			s.x[0] ^= cx;
			STORE(m, mOffset, s.x[0], clen);
			s.x[0] = CLEAR(s.x[0], clen);
			s.x[0] ^= cx;
		}
