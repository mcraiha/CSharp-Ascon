...
namespace tests
{
	public class Ascon128av12Tests
	{
		...
		public void FancyApiTest()
		{
			...
			// Act
			byte[] encrypted16BytesPlusTag = Ascon128av12.Encrypt(messageOf16Bytes, emptyAssociatedData, nonce, key);
			byte[] messageOf16BytesDecrypted = Ascon128av12.Decrypt(encrypted16BytesPlusTag, emptyAssociatedData, nonce, key);

			byte[] encryptedManyBytesPlusTag = Ascon128av12.Encrypt(messageOfManyBytes, longAssociatedData, nonce, key);
			byte[] messageOfManyBytesDecrypted = Ascon128av12.Decrypt(encryptedManyBytesPlusTag, longAssociatedData, nonce, key);

			int func_ret = Ascon128av12.crypto_aead_encrypt(messageOf64BytesEncrypted, out int clen, messageOf64Bytes.ToArray(), 63, longAssociatedData.ToArray(), longAssociatedData.Length, null, nonce.ToArray(), key.ToArray());
		...
		public void NonPowerOfTwoLegacyApiTest()
		{
			...
			byte[] ct = new byte[msg.Length + Ascon128av12.CRYPTO_ABYTES];

			// Act
			int func_ret = Ascon128av12.crypto_aead_encrypt(ct, out int clen, msg, msg.Length, ad, ad.Length, null, nonce, key);
			func_ret = Ascon128av12.crypto_aead_decrypt(msg2, out int mlen2, null, ct, clen, ad, ad.Length, nonce, key);
		...
		public void IncorrectDecryptParametersTest()
		{
			...
			// Act
			var nullMessageException1 = Assert.Throws<NullReferenceException>(() => Ascon128av12.Decrypt(null, associatedDataValid, nonceValid, keyValid) );
			var nullMessageException2 = Assert.Throws<NullReferenceException>(() => Ascon128av12.Decrypt(encryptedMessageValid, null, nonceValid, keyValid) );
			var nullMessageException3 = Assert.Throws<NullReferenceException>(() => Ascon128av12.Decrypt(encryptedMessageValid, associatedDataValid, null, keyValid) );
			var nullMessageException4 = Assert.Throws<NullReferenceException>(() => Ascon128av12.Decrypt(encryptedMessageValid, associatedDataValid, nonceValid, null) );

			var argumentException1 = Assert.Throws<ArgumentException>(() => Ascon128av12.Decrypt(new byte[0], associatedDataValid, nonceValid, keyValid) );
			var argumentException2 = Assert.Throws<ArgumentException>(() => Ascon128av12.Decrypt(encryptedMessageValid, associatedDataValid, new byte[0], keyValid) );
			var argumentException3 = Assert.Throws<ArgumentException>(() => Ascon128av12.Decrypt(encryptedMessageValid, associatedDataValid, nonceValid, new byte[0]) );
